...
void Beveler::DrawOuterBevel(const Rect& rect, int size, Graphics& gfx) const
{
...
void Beveler::DrawInnerBevel(const Rect& rect, int size, Graphics& gfx) const
{
	const int left = int(rect.left);
	const int right = int(rect.right);
	const int top = int(rect.top);
	...
	gfx.DrawRect(right - size, top + size, right, bottom, leftColor);
...
void Beveler::DrawBeveledBrick(Rect& rect, int bevelSize, Graphics& gfx) const
{
	DrawOuterBevel(rect, bevelSize, gfx);
	gfx.DrawRect(rect.GetExpanded(-float(bevelSize)), baseColor);
}

void Beveler::DrawBeveledFrame(Rect& rect, int bevelSize, Graphics& gfx) const
{
	DrawOuterBevel(rect, bevelSize, gfx);
...
void Beveler::SetBaseColor(Color c)
{
	baseColor = c;
	topColor = Color(
